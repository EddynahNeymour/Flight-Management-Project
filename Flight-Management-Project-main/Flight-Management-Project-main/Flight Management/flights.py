...
scheduled = {'AA6564': ["10:10", "Edinburgh", "Delayed"], 'BA585': ["8:00", "Dublin", "Delayed"],
             'AA109': ["1:49", "Boston", "Scheduled"], 'BA1388': ["6:30", "Manchester", "Scheduled"],
             'BA176': ["2:14", "New York", "Delayed"], 'AA6564': ["2:15", "Edinburgh", "Scheduled"],
             'BA1476': ["8:30", "Glasgow", "Scheduled"], 'BA1326': ["3:00", "Newcastle", "Scheduled"]}
cancelled = {'BA1388': ["3:00", "Manchester", "Cancelled"], 'AA109': ["5:20", "Boston", "Cancelled"]}
...
def reminder():
    ...
    for x in scheduled:
        ...
        if time_clock > time_hrs:
            ...
            scheduled_reminders.title("Flight Reminder")
...
def delete_users_account():  
    ...
    admin2 = tk.Button(master=users_delete_acc, text="Confirm Deletion", width=70, command=delete_users_back).grid(row=1, column=2)
...
def user_management():
        ...
        def adding_admin():
            def admin_username():
                ...
                if user in admin or user in manager or user in standard:
                    ...
                    password = tk.Entry(master=system, show='*')
            ...
            tk.Label(master=system, text="Enter Username").grid(row=1, column=0)
            username = tk.Entry(master=system)
            username.grid(row=1, column=1)
            admin3 = tk.Button(master=system, text="Confirm Username", width=70, command=admin_username).grid(row=2, column=3)
        ...
        def add_manager():
            def manager_username():
                ...
                if user in admin3 or user in manager or user in standard:
                    root =tk.Tk()
                    root.title("User Already Exists")
                    tk.Label(master=root, text="Username Already Exists Please Try Again").grid(row=1, column=1)
                    manage_user_scheduled.destroy()
                    system.destroy()
                else:
                    tk.Label(master=system, text="Enter Password").grid(row=2, column=0)
                    password = tk.Entry(master=system, show='*')
                    password.grid(row=2, column=1)
                    tk.Button(master=system, text="Confirm Password", command=manager_users_back, width=70).grid(row=2, column=3)
                ...
                username = tk.Entry(master=system)
                username.grid(row=1, column=1)
                admin3 = tk.Button(master=system, text="Confirm Username", width=70, command=manager_username).grid(row=1, column=3)
        ...
        def adding_standard():
            def standard_username():
                ...
                if user in admin or user in manager or user in standard:
                    ...
                    tk.Button(master=system, text="Confirm Password", command=standard_back_users, width=70).grid(row=2,
                                                                                                                  column=3)
            ...
            username = tk.Entry(master=system)
            username.grid(row=1, column=1)
            admin3 = tk.Button(master=system, text="Confirm Username", width=70, command=standard_username).grid(row=1,
                                                                                                                 column=3)

        adding_users_scheduled = tk.Tk()
        adding_users_scheduled.title("Add A User")
        tk.Button(master=adding_users_scheduled, width=70, text="Add An Admin", command=adding_admin).grid(row=1,
                                                                                                           column=1)
        tk.Button(master=adding_users_scheduled, width=70, text="Add A Supervisor", command=add_manager).grid(row=2,
                                                                                                              column=1)
        tk.Button(master=adding_users_scheduled, width=70, text="Add A Standard User", command=adding_standard).grid(
            row=3, column=1)
    ...
    def deleting_users():
        ...
        username.grid(row=1, column=1)
        admin2 = tk.Button(master=delete_users_scheduled, text="Confirm Deletion", width=70,
                           command=deleting_back_user).grid(row=1, column=2)

    manage_user_scheduled = tk.Tk()
    manage_user_scheduled.title("Manage Users")
    tk.Button(master=manage_user_scheduled, text="Add Users", width=70, command=adding_user).grid(row=1, column=0)
    tk.Button(master=manage_user_scheduled, text="Delete A User", width=70, command=deleting_users).grid(row=2,
                                                                                                         column=0)
    tk.Button(master=manage_user_scheduled, text="View Current Users", width=70, command=display_users).grid(row=3,
                                                                                                             column=0)

#Updates Flights Added
def update():
