...
def delete_users_account():  
    ...
    admin2 = tk.Button(master=users_delete_acc, text="Confirm Deletion", width=25, command=delete_users_back).grid(row=1, column=2)
...
def user_management():
    def adding_user():
        def adding_admin():
            ...
            username.grid(row=1, column=1)
            admin3 = tk.Button(master=system, text="Confirm Username", width=25, command=admin_username).grid(row=2, column=3)
        ...
        def add_manager():
            def manager_username():
                ...
                if user in admin3 or user in manager or user in standard:
                    ...
                    password = tk.Entry(master=system, show='*')
                    password.grid(row=2, column=1)
                    tk.Button(master=system, text="Confirm Password", command=manager_users_back, width=25).grid(row=2, column=3)
                ...
                username = tk.Entry(master=system)
                username.grid(row=1, column=1)
                admin3 = tk.Button(master=system, text="Confirm Username", width=25, command=manager_username).grid(row=1, column=3)
